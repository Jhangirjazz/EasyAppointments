...
App.Pages.Booking = (function () {
    ...
    function initialize() {
        ...
        const savedCategory = localStorage.getItem('ea_selected_category');
        if(savedCategory && !window.App?.Vars?.selectedCategory){
            if(!window.App)window.App = { Vars: {} };
            window.App.Vars.selectedCategory = savedCategory;
        ...
        // FINAL STEP: Update confirm frame if already visible
     if ($('#wizard-frame-4').is(':visible')) {
        setTimeout(() => {
            App.Pages.Booking.updateConfirmFrame();
        }, 100);
    }
    ...
    function addEventListeners() {
         ...
         */
        // === Always keep category in localStorage for persistence ===
$selectService.on('change', function () {
    const serviceId = $(this).val();
    
    // === GET CATEGORY FROM CUSTOM DROPDOWN OR SERVICE DATA ===
    let categoryName = 'Other Services';
    let categoryId = 'uncategorized';
    
    // Method 1: Get from custom dropdown selection
    const $categorySelect = $('#select-category');
    if ($categorySelect.length && $categorySelect.val()) {
        categoryName = $categorySelect.find('option:selected').text();
        categoryId = $categorySelect.val();
    }
    // Method 2: Get from service option data attributes
    else {
        const selectedOption = $(this).find('option:selected');
        categoryName = selectedOption.data('category') || selectedOption.attr('data-category') || 'Other Services';
        categoryId = selectedOption.data('category-id') || selectedOption.attr('data-category-id') || 'uncategorized';
    }
    
    console.log('Category determined:', categoryName, 'ID:', categoryId);
    
    // Save category information
    if (!window.App) window.App = { Vars: {} };
    if (!window.App.Vars) window.App.Vars = {};
    window.App.Vars.selectedCategory = categoryName;
    window.App.Vars.selectedCategoryId = categoryId;
    window.selectedCategory = categoryName;
    localStorage.setItem('ea_selected_category', categoryName);
    localStorage.setItem('ea_selected_category_id', categoryId);
    
    console.log('Category stored on service change:', categoryName);
    
    // Rest of your existing service change logic
    ...
    App.Pages.Booking.updateServiceDescription(serviceId);
        ...
        $selectService.on('change', (event) => {
            ...
            // === SAVE CATEGORY WHEN SERVICE CHANGES ===
            const selectedOption = $selectService.find('option:selected');
            const categoryName = selectedOption.data('category') || selectedOption.attr('data-category');
    ...
    if (categoryName) {
        ...
        window.App.Vars.selectedCategory = categoryName;
        window.selectedCategory = categoryName;
        console.log('CATEGORY SET ON CHANGE:', categoryName);
    }
    ...
    $(event.currentTarget)
        ...
        .fadeOut(() => {
            ...
            $('#wizard-frame-' + prevTabIndex).fadeIn(() => {
                // Update category when going back to step 4
                if (prevTabIndex === 4) {
                    setTimeout(() => {
                        App.Pages.Booking.updateConfirmFrame();
                    }, 100);
  ...
  $('.button-next').on('click', (event) => {
    ...
    if ($target.attr('data-step_index') === '1' && !$selectProvider.val()) return;
    if ($target.attr('data-step_index') === '2' && !$('.selected-hour').length) {
    ...
    if ($target.attr('data-step_index') === '3' && !App.Pages.Booking.validateCustomerForm()) return;
    ...
    $target.parents().eq(1).fadeOut(() => {
        ...
        $('#wizard-frame-' + nextTabIndex).fadeIn(() => {
            // IMPORTANT: Add delay to ensure DOM is ready
            setTimeout(() => {
                App.Pages.Booking.updateConfirmFrame();
            }, 100);           
        });
    ...
    if (window.innerHeight < scrollingElement.scrollHeight) scrollingElement.scrollTop = 0;
});
        // $('.button-next').on('click', (event) => {
        //     const $target = $(event.currentTarget);
    ...
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                const display = window.getComputedStyle(wizardFrame4).display;
                if (display !== 'none') {
                    console.log('Wizard frame 4 became visible, updating category...');
                    setTimeout(() => {
                        App.Pages.Booking.updateConfirmFrame();
                    }, 150);
                }
...
//     console.log('updateConfirmFrame called');

// // === RECOVER CATEGORY FROM ANY SOURCE ===
...
// // Ensure App.Vars is set
//     if (!window.App) window.App = { Vars: {} };
//     if (!window.App.Vars) window.App.Vars = {};
//     window.App.Vars.selectedCategory = selectedCategory;

//     console.log('Selected Category',selectedCategory);

//     // === ALWAYS UPDATE CATEGORY (even before time is picked) ===
//     $('#category-display').text(selectedCategory);

//     const serviceId = $selectService.val();
//     const providerId = $selectProvider.val();

// $displayBookingSelection.text(`${lang('service')} │ ${lang('provider')}`);
...
//     // Only return early if we need a time AND it's missing
//     if (!$('.selected-hour').length) {
//         // Clear appointment details but keep category
...
//         ? App.Utils.Date.format(selectedDateMoment.format('YYYY-MM-DD'), vars('date_format'), vars('time_format'), false) + ' ' + selectedTime
//         : '';
...
// // === 2. RENDER ONLY THE REST (without category) ===
// $('#appointment-details').html(`
//     <div class="mb-2 fw-bold fs-3">${serviceOptionText}</div>
//     <div class="mb-2 fw-bold text-muted">${providerOptionText}</div>
//     <div class="mb-2"><i class="fas fa-calendar-day me-2"></i>${formattedSelectedDate}</div>
//     <div class="mb-2"><i class="fas fa-clock me-2"></i>${service.duration} ${lang('minutes')}</div>
//     <div class="mb-2"><i class="fas fa-globe me-2"></i>${timezoneOptionText}</div>
...
//     </div>
// `);
//   // ONE FINAL .html() CALL — NO DUPLICATES, NO UNDEFINED
// // $('#appointment-details, #appointment-details-content').html(`
// //   <div>
// //     <!-- SERVICE -->
...
//             <div class="mb-2 fw-bold text-muted" ${!fullName ? 'hidden' : ''}>${fullName}</div>
//             <div class="mb-2" ${!email ? 'hidden' : ''}>${email}</div>
//             <div class="mb-2" ${!phoneNumber ? 'hidden' : ''}>${phoneNumber}</div>
//             <div class="mb-2" ${!address ? 'hidden' : ''}>${address}</div>
//             <div class="mb-2" ${!addressParts.length ? 'hidden' : ''}>${addressParts.join(', ')}</div>
//         </div>
//     `);

function updateConfirmFrame() {
    console.log('updateConfirmFrame called - checking category');
    
    // === 1. IMPROVED CATEGORY RECOVERY ===
    let selectedCategory = 'Category 2'; // Default fallback
    
    // Check ALL possible sources in order of priority
    if (window.App?.Vars?.selectedCategory) {
        selectedCategory = window.App.Vars.selectedCategory;
        console.log('Category from App.Vars:', selectedCategory);
    } else if (window.selectedCategory) {
        selectedCategory = window.selectedCategory;
        console.log('Category from window.selectedCategory:', selectedCategory);
    } else if (localStorage.getItem('ea_selected_category')) {
        selectedCategory = localStorage.getItem('ea_selected_category');
        console.log('Category from localStorage:', selectedCategory);
    } else {
        console.log('No category found, using default');
    }
    
    // Ensure App.Vars is ALWAYS set with the current category
    if (!window.App) window.App = { Vars: {} };
    if (!window.App.Vars) window.App.Vars = {};
    window.App.Vars.selectedCategory = selectedCategory;
    
    console.log('Final category to display:', selectedCategory);

    // === 2. BRUTE FORCE CATEGORY UPDATE ===
    const bruteForceCategoryUpdate = () => {
        const categoryEl = document.getElementById('category-display');
        if (categoryEl) {
            categoryEl.textContent = selectedCategory;
            categoryEl.innerText = selectedCategory;
            console.log('🔧 Category element updated to:', selectedCategory);
        } else {
            console.log('Category display element not found');
        }
    };

    // === 3. EXECUTE IMMEDIATELY ===
    bruteForceCategoryUpdate();
    ...
    // === 5. BUILD APPOINTMENT HTML WITH CATEGORY ===
    let appointmentHtml = `
        <div class="mb-2">
            <i class="fas fa-tags me-2"></i>
            <div id="category-display" class="mb-3 fw-bold text-uppercase" style="color: #ff6b6b; font-size: 1.4rem; letter-spacing: 1px;">
                ${selectedCategory}
            </div>
        </div>
        <div class="mb-2 fw-bold fs-3">${serviceOptionText}</div>
    ...
    if ($('.selected-hour').length && service) { 
        const selectedDateObject = App.Utils.UI.getDateTimePickerValue($selectDate);
        const selectedDateMoment = moment(selectedDateObject);
        const selectedTime = $availableHours.find('.selected-hour').text();
        const formattedSelectedDate = selectedDateObject
    ...
    // === 6. SET APPOINTMENT DETAILS ===
    console.log('Setting appointment details HTML...');
    $('#appointment-details').html(appointmentHtml);

    // === 7. FORCE CATEGORY UPDATES MULTIPLE TIMES ===
    console.log('Scheduling category updates...');
    setTimeout(bruteForceCategoryUpdate, 50);
    setTimeout(bruteForceCategoryUpdate, 100);
    setTimeout(bruteForceCategoryUpdate, 200);
    setTimeout(bruteForceCategoryUpdate, 500);
    setTimeout(bruteForceCategoryUpdate, 1000);

    // === 8. AGGRESSIVE PROTECTION ===
    if (window.categoryProtectionInterval) {
        clearInterval(window.categoryProtectionInterval);
    }
    
    window.categoryProtectionInterval = setInterval(() => {
        if ($('#wizard-frame-4').is(':visible')) {
            console.log('Category protection running...');
            bruteForceCategoryUpdate();
            
            // Also re-check the category source
            const currentCategory = window.App?.Vars?.selectedCategory || 
                                  window.selectedCategory || 
                                  localStorage.getItem('ea_selected_category') || 
                                  'Category 2';
            if (currentCategory !== selectedCategory) {
                console.log('🔄 Category changed during protection:', currentCategory);
    ...
    console.log('updateConfirmFrame completed');

    // === 10. POST DATA UPDATES ===
    const data = {
        customer: {
        ...
        appointment: {
            start_datetime: moment(App.Utils.UI.getDateTimePickerValue($selectDate)).format('YYYY-MM-DD') +
